...
def read_xml_body():
    ...
    for enc in ("utf-8", "cp1252", "latin-1", "iso-8859-1"):
        try:
            return raw.decode(enc), is_hex_param
        except Exception:
            continue
...
def sh_headers():
    if not SHOPIFY_TOKEN:
        raise RuntimeError("SHOPIFY_TOKEN not set")
...
def shopify_find_variant_by_sku(sku):
    """
    Robustly find a variant by exact SKU.
    1) Try /variants.json?sku=... and verify the match.
    2) If API ignores the filter, fall back to paginated scan of variants.
    Cap the scan for safety/perf with FIND_SKU_MAX_VARIANTS.
    """
    base = shopify_base()
    headers = sh_headers()
    target = (sku or "").strip()
    ...
    # Fallback: paginated scan over variants
    scanned = 0
    since_id = 0
...
def shopify_update_product(pid, p):
    r = requests.put(
        f"{shopify_base()}/products/{pid}.json", headers=sh_headers(), data=json.dumps({"product": p}), timeout=60
    )
    if r.status_code not in (200, 201):
...
def shopify_update_variant(vid, payload):
    ...
    if r.status_code not in (200, 201):
        raise RuntimeError(f"variant {r.status_code}: {r.text[:300]}")
...
def shopify_set_inventory(iid, qty):
    ...
    if r.status_code not in (200, 201):
        raise RuntimeError(f"inventory {r.status_code}: {r.text[:300]}")
    return r.json()
    ...
    try:
        for key, val in meta.items():
            if val is None or val == "":
                continue
            body = {"metafield": {"namespace": "uni", "key": key, "value": str(val), "type": "single_line_text_field"}}
            requests.post(
...
def shopify_set_seo(pid: int, title: str | None = None, desc: str | None = None):
    try:
        for key, val in (("title_tag", title), ("description_tag", desc)):
            ...
            if not val:
                continue
            body = {"metafield": {"namespace": "global", "key": key, "type": "single_line_text_field", "value": val}}
            requests.post(
                f"{shopify_base()}/products/{int(pid)}/metafields.json", headers=sh_headers(), data=json.dumps(body), timeout=30
            )
    except Exception as e:
...
def _log_req():
    try:
        ...
        log.info("REQ %s %s?%s  Referer=%s", request.method, request.path, request.query_string.decode(errors="ignore"), ref)
    except Exception:
        pass
...
def post_product_group():
    ...
    conn = db()
    groups = (
        root.findall(".//productgroup")
        or root.findall(".//group")
        or root.findall(".//gruppe")
        or root.findall(".//varegruppe")
    )
    for g in groups:
        gid = (
            g.findtext("groupno")
            or g.findtext("groupid")
            or g.findtext("id")
            or g.findtext("gruppeid")
            or g.findtext("grpid")
            or ""
        ).strip()
        gname = (
            g.findtext("description")
            or g.findtext("groupname")
            or g.findtext("name")
            or g.findtext("gruppenavn")
            or ""
        ).strip()
        parent = (
            g.findtext("parentgroup") or g.findtext("parentid") or g.findtext("parent") or g.findtext("overgruppeid") or ""
        ).strip()
        if not gid:
            continue
        conn.execute(
...
def post_product():
    ...
    nodes = root.findall(".//product") or root.findall(".//vare") or root.findall(".//item") or root.findall(".//produkt")
    if not nodes:
        cands = []
        idtags = ["productident", "prodid", "varenr", "sku", "itemno"]
    ...
    for p in nodes:
        ...
        longdesc = (
            findtext_ci_any(
                p, ["longdesc", "longtext", "description_long", "desc", "body_html", "infohtml", "produktbeskrivelse"]
            )
            or ""
        )
        ...
        vendor = (
            findtext_ci_any(p, ["vendor", "produsent", "leverandor", "leverandør", "manufacturer", "brand", "supplier"]) or ""
        ).strip()
        ...
        seo_title = (
            SEO_DEFAULT_TITLE_TEMPLATE.format(title=full_title, sku=sku, vendor=vendor) if SEO_DEFAULT_TITLE_TEMPLATE else None
        )
        seo_desc = (
            SEO_DEFAULT_DESC_TEMPLATE.format(title=full_title, sku=sku, vendor=vendor) if SEO_DEFAULT_DESC_TEMPLATE else None
        )

        log.info(
            "PARSED sku=%s title=%r price=%s (src=%s) ordinary=%s stock=%s vendor=%r group=%r",
            sku,
            full_title,
            price,
            price_src,
        ...
        try:
            ...
            if v:
                ...
                log.info(
                    "Shopify UPDATE OK sku=%s product_id=%s admin=https://%s/admin/products/%s",
                    sku,
                    pid,
                    SHOPIFY_DOMAIN,
                    pid,
                ...
                iid = created["variants"][0]["inventory_item_id"]
                log.info(
                    "Shopify CREATE OK sku=%s product_id=%s status=%s admin=https://%s/admin/products/%s",
                    sku,
                    pid,
                    cp["status"],
                    SHOPIFY_DOMAIN,
...
def bare_asp_placeholder():
    logging.warning(
        "Placeholder ASP hit. Likely a blank filename in Uni Sideadministrasjon. QS=%s",
        request.query_string.decode("utf-8", "ignore"),
    )
...
def twinxml_fallback(rest):
    ...
    path_l = rest.lower()
    is_upload = (
        path_l.startswith("post")
        or "product" in path_l
        or "item" in path_l
        or "price" in path_l
        or "stock" in path_l
        or "inventory" in path_l
        or "discount" in path_l
    )
